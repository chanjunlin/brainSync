...
class Explain extends events_1.default {
    constructor(appmapDirectories, projectDirectories, question, codeSelection, appmaps, status, codeEditor, prompt) {
        super();
    ...
    async searchContext(data) {
        ...
        const labels = (_a = data.labels) !== null && _a !== void 0 ? _a : [];
        if (!tokenCount) {
        ...
        const searchResult = await (0, collectContext_1.default)(this.appmapDirectories.map((dir) => dir.directory), this.projectDirectories, this.appmaps, keywords, charLimit, data);
...
const codeSelections = new lru_cache_1.LRUCache({
    maxSize: 1024 ** 2 * 10 /* 10 MB */,
    sizeCalculation: (value) => value.length,
});
async function explain(navieProvider, question, codeSelection, appmaps, threadId, codeEditor, prompt) {
    const status = {
    ...
    const explain = new Explain(appmapDirectories, projectDirectories, question, cachedCodeSelection, appmaps, status, codeEditor, prompt);
    ...
    return new Promise((resolve, reject) => {
        ...
        explain.on('ack', (userMessageId, threadId) => {
            ...
            if (codeSelection)
                codeSelections.set(threadId, codeSelection);
...
const explainHandler = (navieProvider, codeEditor) => {
    return {
        ...
        handler: async (options) => await explain(navieProvider, options.question, options.codeSelection, options.appmaps, options.threadId, codeEditor, options.prompt),
    };
};
