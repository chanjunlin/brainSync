...
class EventCollector {
    ...
    async collectEvents(maxEvents, excludePatterns, includePatterns, includeTypes) {
        ...
        const isIncludedType = (item) => {
            if (includeTypes && !includeTypes.some((type) => type === item.type))
                return false;
            return true;
        };
        const context = (await (0, buildContext_1.default)(results)).filter(isIncludedType);
    ...
    async findEvents(appmap, options) {
...
class ContextCollector {
    ...
    async collectContext() {
        if (this.vectorTerms.length === 0 || this.vectorTerms.every((term) => term.trim() === '')) {
            (0, console_1.warn)('No vector terms received. Context search will be skipped.');
        ...
        const fileSearchResponse = await (0, withIndex_1.default)('files', (indexFileName) => (0, FileIndex_1.buildFileIndex)(this.sourceDirectories, indexFileName, this.excludePatterns, this.includePatterns), (index) => index.search(this.vectorTerms, search_1.DEFAULT_MAX_FILES));
...
async function collectContext(appmapDirectories, sourceDirectories, appmaps, vectorTerms, charLimit, filters) {
    ...
    if (filters === null || filters === void 0 ? void 0 : filters.exclude)
    ...
    if (filters === null || filters === void 0 ? void 0 : filters.itemTypes)
        contextCollector.includeTypes = filters.itemTypes.map((type) => type);
