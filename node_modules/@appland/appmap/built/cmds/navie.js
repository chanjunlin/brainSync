...
async function handler(argv) {
    ...
    let codeSelection;
    if (argv.codeSelection)
        codeSelection = await (0, promises_1.readFile)(argv.codeSelection, 'utf-8');
    let prompt;
    if (argv.prompt)
        prompt = await (0, promises_1.readFile)(argv.prompt, 'utf-8');
    ...
    if (argv.directory.length === 1)
        process.chdir(argv.directory[0]);
    await (0, explain_1.explainHandler)(capturingProvider, codeEditor).handler({ question, codeSelection, prompt });
}
...
async function getQuestion(path, literal) {
    ...
    if (targetPath === '-') {
        ...
        question.push(await (0, consumers_1.text)(process.stdin));
    }
    else if (targetPath) {
        (0, node_console_1.warn)(`Reading question from ${targetPath}`);
        question.push(await (0, promises_1.readFile)(targetPath, 'utf-8'));
    }
